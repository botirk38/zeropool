...
fn main() {
    ...
    println!(
        ...
        pool.config.max_buffers_per_shard
    );
    ...
    println!(
        "  max_buffers_per_shard: {}",
        custom_pool.config.max_buffers_per_shard
    );
    println!(
        "  min_buffer_size: {} bytes",
        custom_pool.config.min_buffer_size
    );

    let weird_config = PoolConfig::default().with_num_shards(100);
    println!("\nWeird config (requested 100 shards):");
    println!(
        "  actual num_shards: {} (rounded to power-of-2, clamped to 128 max)",
        weird_config.num_shards
    );
}
